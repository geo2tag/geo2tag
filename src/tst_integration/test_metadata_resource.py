...
TEST_URL = '/instance/service/' + \
    'testservice/metadata/552833515404411781370711'
TEST_URL_DELETE = '/instance/service/' + \
    'testservice/metadata/552833515404411781370722'
VALID_RESPONSE_CODE = 200
NOT_VALID_URL = '/instance/service' + \
...
class TestMetadataResource(BasicIntegrationTest):
    ...
    def testMetadataResourceGet(self):
        ...
        responseCode = response.status_code
        self.assertEquals(responseCode, NOT_VALID_RESPONSE_CODE_NOT_FOUND)
    ...
    def testMetadataResourcePut(self):
        ...
        response = requests.put(
            self.getUrl(NOT_VALID_URL),
            data=NOT_VALID_LIST_ARGS)
        responseCode = response.status_code
        self.assertEquals(responseCode, NOT_VALID_RESPONSE_CODE)

    def testMetadataResourceDelete(self):
        response = requests.delete(self.getUrl(TEST_URL_DELETE))
        responseCode = response.status_code
        self.assertEquals(responseCode, VALID_RESPONSE_CODE)
        response = requests.delete(self.getUrl(NOT_VALID_URL))
        responseCode = response.status_code
        self.assertEquals(responseCode, NOT_VALID_RESPONSE_CODE_NOT_FOUND)
