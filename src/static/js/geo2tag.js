function Geo2TagRequests(server, instance) {
	...
	this.getChannels = function(serviceName, callbacksuccess, callbackfail, substring, number, offset) {
		$.get(getUrlWithPrefix('/service/') + serviceName + '/channel', {'substring': substring, 'number': number, 'offset': offset})
			.fail(callbackfail).done(function(){ callbacksuccess(data) });
	};

	this.getPoints = function(serviceName, callbacksuccess, callbackfail, channel_ids, number, geometry, altitude_from, 
