...
class GeocoderRequestOtherExceed(BaseGeo2TagException):
    ...
    def __init__(self, error_code):
        ...
        if error_code == unicode(self.ERROR_LIST_OTHER_EXCEED[0]):
            self.message_error = 'Authorization Exception'
        if error_code == unicode(self.ERROR_LIST_OTHER_EXCEED[1]):
            self.message_error = 'Record does not exist'
        if error_code == unicode(self.ERROR_LIST_OTHER_EXCEED[2]):
            self.message_error = 'Other error'
        if error_code == unicode(self.ERROR_LIST_OTHER_EXCEED[3]):
            self.message_error = 'Database timeout'
        if error_code == unicode(self.ERROR_LIST_OTHER_EXCEED[4]):
            self.message_error = 'Invalid parameter'
        if error_code == unicode(self.ERROR_LIST_OTHER_EXCEED[5]):
            self.message_error = 'No result found'
        if error_code == unicode(self.ERROR_LIST_OTHER_EXCEED[6]):
            self.message_error = 'Duplicate exception'
        if error_code == unicode(self.ERROR_LIST_OTHER_EXCEED[7]):
            self.message_error = 'Postal code not found'
        if error_code == unicode(self.ERROR_LIST_OTHER_EXCEED[8]):
            self.message_error = 'Invalid input'
        if error_code == unicode(self.ERROR_LIST_OTHER_EXCEED[8]):
            self.message_error = 'Server overloaded exception'
        if error_code == unicode(self.ERROR_LIST_OTHER_EXCEED[10]):
